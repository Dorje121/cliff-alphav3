...
const LensSlider: React.FC = () => {
    ...
    return (
        <> <section className="  px-7">
            ...
            <div className="relative bg-black text-white min-h-screen    flex flex-col justify-center overflow-hidden !h-fit md:py-0 py-16">
                <div className="flex flex-col md:flex-row items-start gap-12 transition-all duration-700">
                    ...
                    <div className="w-full md:mt-12 md:w-1/2 text-center md:text-left">
                        <AnimatePresence mode="wait" custom={direction}>
                            <motion.div
                                ...
                                transition={{ duration: 0.6 }}
                            >
                                <h2 className="text-xl sm:text-2xl md:text-4xl lg:text-6xl font-bold mb-6 md:mb-8 !bg-gradient-to-r !from-white !via-yellow-100 !to-yellow-200 !bg-clip-text !text-transparent px-2">
                                    {lensData[current].title}
                                </h2>

                                <p className="text-gray-300 mb-4 text-[1.1rem] sm:text-[1.4rem]">
                                    {lensData[current].desc}
                                </p>
                                <p className="mb-2 ">
                                    <div className="font-semibold text-lg sm:text-2xl">Power: </div>
                                    <div className="text-xl text-gray-300 ">{lensData[current].power}</div>
                                </p>
                                <p className="mb-2">
                                    <span className="font-semibold text-lg sm:text-2xl">Index: </span>
                                    <span className="text-xl text-gray-300 ">{lensData[current].index}</span>
                                </p>
                                <p>
                                    <span className="font-semibold text-lg sm:text-2xl">Features: </span>
                                    <span className="text-xl text-gray-300">{lensData[current].feature}</span>
                                </p>
                            </motion.div>
                        </AnimatePresence>
                    </div>
                ...
                <div className="flex justify-center mt-6 space-x-3">
                    {lensData.map((_, idx) => (
                        <button
                            ...
                            className={`h-2  w-2 md:mt-0 mt-10 rounded-full ${current === idx ? "bg-white" : "bg-gray-600"
                                }`}
                        />
                    ))}
                </div>
