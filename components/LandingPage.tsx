...
const LandingPage: React.FC<LandingPageProps> = ({ onComplete }) => {
  const [showWelcomeModal, setShowWelcomeModal] = useState(false); // Will be set based on session check
  ...
  useEffect(() => {
    // Clear any existing session storage that might conflict
    sessionStorage.removeItem('cliff_landing_seen');

    // Use localStorage to be consistent with the main page logic
    const hasVisited = localStorage.getItem('cliff-has-visited');

    if (!hasVisited) {
      // First time visiting - show landing page
      setShouldShowLanding(true);
      setShowWelcomeModal(true);
    } else {
      // Already visited before - skip landing page
      setShouldShowLanding(false);
      // Immediately complete the landing to show main content
      setTimeout(() => {
        onComplete?.();
      }, 100);
    }
  ...
  const startBackgroundAudio = () => {
    console.log('Starting background audio (bg.mp3)...');
    ...
    try {
      ...
      bgAudio.play().then(() => {
        console.log('Background audio (bg.mp3) playing successfully');
      ...
      console.log('Background audio creation failed:', error);
    }
  };
  ...
  const handleSoundPreference = (enableSound: boolean) => {
    ...
    if (enableSound) {
      ...
      // Create and play main audio (audio.wav) first
      try {
        const audio = new Audio('/audio/audio.wav');
        audio.volume = 0.5;
        setMainAudio(audio);

        // Play main audio immediately - this should work because it's in response to user click
        audio.play().then(() => {
          console.log('Main audio (audio.wav) playing successfully');
          setAudioPlaying(true);
          setShowAudioContent(true);

          // When main audio ends, start background audio (bg.mp3)
          audio.onended = () => {
            console.log('Main audio finished, starting background audio and shutter animation');
            startBackgroundAudio(); // This will play bg.mp3 in loop
            setIsFadingOut(true);
            handleLandingComplete();
          };
        }).catch((error: unknown) => {
          console.log('Main audio play failed:', error);
          // If main audio fails to play, continue with the experience
        ...
        console.log('Main audio creation failed:', audioError);
      ...
      // User declined sound - clean up any existing audio
      if (backgroundAudio) {
        backgroundAudio.pause();
        backgroundAudio.currentTime = 0;
        backgroundAudio.src = '';
  ...
  const handleLandingComplete = () => {
    // Don't stop background audio here - let it continue playing during transition
    // Only stop main audio if it's still playing
    if (mainAudio) {
      mainAudio.pause();
      mainAudio.currentTime = 0;
      mainAudio.src = '';
    ...
    // Remove main audio element but keep background audio playing
    const mainAudioElements = document.querySelectorAll('audio[src*="audio.wav"]');
    mainAudioElements.forEach(element => {
      const audioElement = element as HTMLAudioElement;
      audioElement.pause();
      audioElement.currentTime = 0;
      audioElement.src = '';
  ...
  return (
    <>
      {/* Only render the landing page if it should be shown */}
      {shouldShowLanding && (
        <motion.div
