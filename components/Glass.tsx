...
import React, { useEffect, useRef, Suspense, MutableRefObject } from "react";
import { gsap } from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import { motion } from "framer-motion";
import * as THREE from "three";
...
function GlassesModel({ groupRef }: GlassesModelProps) {
  const gltf = useGLTF("/models/jump.glb") as unknown as GLTF;
  const [chasmaSize, setChasmaSize] = React.useState(44);
      ...
      gltf.scene.traverse((child) => {
        if (child instanceof THREE.Mesh) {
          child.material = new THREE.MeshPhysicalMaterial({
            ...
            transparent: true,
            opacity: 1, // full clear
            roughness: 0, // super smooth
            metalness: 0, // no metal
            transmission: 1, // full glass effect
            ior: 1.5, // index of refraction for glass
            thickness: 0.4, // refraction thickness
            reflectivity: 0.5, // how reflective
            envMapIntensity: 1,
            clearcoat: 1,
            clearcoatRoughness: 0,
          });

          // Optional: cast & receive shadows if needed
